...
function ModelDonut({ session: s }: { session: Session }) {
  ...
  return (
    ...
    >
      ...
      {tip && s.ctx_used && (
        <div
          className="fixed z-50 -translate-x-1/2 px-2 py-1 bg-[#171717] text-canvas text-[10px] rounded shadow whitespace-nowrap tabular-nums pointer-events-none"
...
function Th({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <th
      className={
        "px-3 sm:px-[14px] py-[9px] text-left text-[10px] uppercase tracking-[.09em] text-[#a3a3a3] font-medium bg-canvas " +
        className
      }
    >
      {children}
