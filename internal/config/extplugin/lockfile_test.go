...
func TestLockStoreSourceRoundTrip(t *testing.T) {
	...
	s.setSource("gh", "github.com/acme/p", "v1.2.3", "~> 1.2")
	s.addHash("gh", "sha256:abc", "outbound")
	if err := s.save(); err != nil {
		t.Fatal(err)
	}
	raw := readString(t, path)
	for _, want := range []string{`source      = "github.com/acme/p"`, `version     = "v1.2.3"`, `constraints = "~> 1.2"`} {
	...
	if !ok || e.Source != "github.com/acme/p" || e.Version != "v1.2.3" ||
		e.Constraints != "~> 1.2" || e.Network != "outbound" || !e.hasHash("sha256:abc") {
