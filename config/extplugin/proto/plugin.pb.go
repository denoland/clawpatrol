...
type EvaluateAction struct {
	...
	// unparseable signals that the plugin's parser refused the
	// inbound payload — analogous to match.Request.Unparseable on
	// built-in facets (clickhouse_native sets it when its SQL parser
	// can't reduce the wire bytes to a verb / tables / functions
	// tuple). When true, the plugin facet's CEL `unparseable`
	// binding evaluates to true and the dispatcher's fail-closed
	// gate (config/runtime/dispatch.go) synth-denies any rule whose
	// condition reads it. Plugins that have no parser stage (their
	// action_json IS the schema-validated payload) leave this false.
	Unparseable   bool `protobuf:"varint,6,opt,name=unparseable,proto3" json:"unparseable,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *EvaluateAction) GetUnparseable() bool {
	if x != nil {
...
const file_plugin_proto_rawDesc = "" +
	...
	"\x06handle\x18\x01 \x01(\tR\x06handle\"\xae\x02\n" +
	...
	"\astreams\x18\x05 \x03(\v21.clawpatrol.plugin.v1.EvaluateAction.StreamsEntryR\astreams\x12 \n" +
	"\vunparseable\x18\x06 \x01(\bR\vunparseable\x1a:\n" +
