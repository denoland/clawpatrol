...
| `insecure_no_dashboard_auth` | `bool` | no | Disables dashboard authentication entirely — every request is treated as if the root password principal were present. Dev convenience for `clawpatrol gateway dev.hcl` iteration. Loopback-bind only: the gateway refuses to start when this is true unless InfoListen resolves to a loopback host (127.0.0.1 / ::1). Never set in production. |
