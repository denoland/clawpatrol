...
upstream_called, grant_consumed_ns, grant_consumed_by, slack_message_ref, dashboard_ref, last_error`
...
func (s *HITLOperationStore) scanOne(ctx context.Context, query string, args ...any) (HITLOperation, error) {
	...
	var expiredReason, grantConsumedBy, slackMessageRef, dashboardRef, lastError sql.NullString
	...
	err := row.Scan(
		...
		&upstreamCalled, &grantConsumedNS, &grantConsumedBy, &slackMessageRef, &dashboardRef, &lastError,
	)
	...
	op.SlackMessageRef = slackMessageRef.String
